...
// --- MAIN COMPONENT ---
export default function TrainerImportProgramPage() {
  ...
  const handleImport = async () => {
    if (!trainer || !firestore || validRows.length === 0 || !tenGiaoAn) {
    ...
    try {
        const trainerTemplatesRef = collection(firestore, 'programTemplatesTrainer', trainer.uid, 'items');
        
        const newTemplateDocRef = doc(trainerTemplatesRef);
    ...
    <div className="space-y-8">
      <div className="flex items-center gap-4">
        <Button variant="outline" size="icon" asChild>
            <Link href="/trainer/library/programs"><ArrowLeft className="h-4 w-4" /></Link>
        </Button>
        <div>
            <h1 className="text-3xl font-bold font-headline">Import Giáo án từ File</h1>
            <p className="text-muted-foreground">Tải lên một file CSV hoặc Excel để tạo hàng loạt giáo án mẫu cho kho riêng của bạn.</p>
        </div>
      </div>
