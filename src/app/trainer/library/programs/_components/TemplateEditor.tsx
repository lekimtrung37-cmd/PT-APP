...
export function TemplateEditor({ templateId, isPublic = false }: TemplateEditorProps) {
  const { user } = useUser();
  ...
  const isReadOnly = isPublic && userRole === 'pt';

  const templateCollectionPath = useMemoFirebase(() => {
    if (isPublic) return 'programTemplatesPublic';
    if (!user) return null;
    return `programTemplatesTrainer/${user.uid}/items`;
  }, [isPublic, user]);
  ...
  const { data: publicExercises, isLoading: arePublicExercisesLoading } = useCollection<{ id: string, ten: string }>(publicExercisesQuery);
  ...
  const { data: trainerExercises, isLoading: areTrainerExercisesLoading } = useCollection<{ id: string, ten: string }>(trainerExercisesQuery);
  ...
  const exerciseLibrary = useMemo(() => {
    ...
    const unique = Array.from(new Map(combined.map(item => [item['ten'], item])).values());
    return unique.sort((a,b) => a.ten.localeCompare(b.ten));
  ...
  const handleUpdateTemplateName = async () => {
    if (!templateDocRef || !templateName.trim() || isReadOnly) return;
  ...
  const handleDeleteSession = async (sessionId: string) => {
    ...
    if (!firestore || !user || !template) return;
    setIsSubmitting(true);

    const newName = `(Bản sao) ${template.ten}`;
    const trainerTemplatesColRef = collection(firestore, `programTemplatesTrainer/${user.uid}/items`);
    ...
    try {
      ...
      const newTemplateDocRef = await addDoc(trainerTemplatesColRef, newTemplateData);
      
      if (sessions && sessions.length > 0) {
        const batch = writeBatch(firestore);
        const newSessionsColRef = collection(newTemplateDocRef, 'sessions');
        sessions.forEach(session => {
          const { id, ...sessionData } = session; // Exclude original id
          const newSessionDocRef = doc(newSessionsColRef);
          batch.set(newSessionDocRef, sessionData);
        });
        await batch.commit();
      }
      ...
      toast({
        ...
        description: `Giáo án "${newName}" đã được thêm vào thư viện của bạn.`,
      });
  ...
  const SessionFormWrapper = ({ session }: { session: any }) => {
    ...
    const onSessionSubmit = async (data: SessionFormData) => {
        if (!templateDocRef || isReadOnly) return;
        ...
        <Form {...sessionForm}>
            <form onSubmit={sessionForm.handleSubmit(onSessionSubmit)} className="space-y-6">
                 <div className="space-y-6">
                    <SectionForm control={sessionForm.control} name="exercises" label="Bài tập" fields={exerciseFields} append={appendExercise} remove={removeExercise} move={moveExercise} isReadOnly={isReadOnly} exerciseLibrary={exerciseLibrary} openPopoverIndex={openPopoverIndex} setOpenPopoverIndex={setOpenPopoverIndex} />
                    <FormField control={sessionForm.control} name="ghiChuPT" render={({ field }) => (<FormItem><FormLabel>Ghi chú cho buổi tập</FormLabel><FormControl><Textarea placeholder="Lưu ý chung cho cả buổi tập..." {...field} value={field.value ?? ''} readOnly={isReadOnly}/></FormControl></FormItem>)} />
                </div>
                {!isReadOnly && (
  ...
  return (
    <div className="w-full">
        <div className="flex items-center gap-4 mb-4">
            ...
            {isReadOnly && (
              <Button variant="outline" size="sm" onClick={handleDuplicateTemplate} disabled={isSubmitting}>
        ...
        {isReadOnly && (
            <Alert className="mb-4 border-primary bg-primary/10">
                ...
                <AlertDescription>
                    Đây là giáo án mẫu công khai. Bạn không thể chỉnh sửa trực tiếp. Để sử dụng, hãy sao chép nó về thư viện của bạn.
                </AlertDescription>
            </Alert>
        )}
